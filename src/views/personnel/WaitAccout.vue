<template>
  <CRow>
    <CCol :xs="12">
      <CCard class="mb-4">
        <CCardHeader>
          <strong>Danh sách tài khoản nhân viên Chờ xác thực</strong>
        </CCardHeader>
        <CCardBody>
          ...
          <v-dialog v-model="isDialogOpen" max-width="600">
            <v-card>
              ...
              <v-card-actions>
                <v-btn color="blue darken-1" text @click="DestroyUser(selectedUser.id)"
                  >Cho phép hoạt động</v-btn
                >
                <v-spacer></v-spacer>
...
<script>
import { ref } from 'vue'
import { get_grapUsers_byStatus, update_status_userGrap } from '../../api/ApiRequest'
...
export default {
  ...
  setup() {
    ...
    const getData = async (page) => {
      const response = await get_grapUsers_byStatus('wait', page)
    ...
    const DestroyUser = (id) => {
      if (confirm('Xác nhận cho phép tài khoản này hoạt động')) {
        update_status_userGrap(id, 'working').then(() => window.location.reload())
      }
    }
    return {
      users,
